...
function normalizeSettings(s) {
  ...
  return { colorTheme: s?.colorTheme ?? "vivid", wallCount, shelvesPerWall, slotsPerShelf, wallNames };
}
    ...
    <div className="grid gap-4">
      <Section title="Backup & restore">
        <div className="flex flex-wrap items-center gap-3">
          <ToolbarButton label="Export JSON" onClick={exportData} className="bg-indigo-100 text-indigo-800" />
          ...
          <ToolbarButton label="Reset all data" onClick={() => window.confirm("This will erase everything. Proceed?") && dispatch({ type: "reset" })} className="bg-rose-100 text-rose-800" />
        </div>
      </Section>
      <Section title="Tips">
        <ul className="list-disc ml-6 text-sm opacity-80 grid gap-1">
          <li>Everything you add saves to your device via localStorage. Use Export JSON to back up or migrate.</li>
          <li>Drag a polish card (bottom of Wall view) onto any shelf slot to place it. Swap by dropping onto an occupied slot.</li>
          <li>Use tags like <em>favorite</em>, <em>spring</em>, <em>office-safe</em> to power fast filtering.</li>
        </ul>
      </Section>
    </div>
  );
